...
iptables -t mangle -A V2RAY -d 10.0.0.0/8 -j RETURN # 直连局域网，避免 V2Ray 无法启动时无法连网关的 SSH，如果你配置的是其他网段（如 10.x.x.x 等），则修改成自己的
iptables -t mangle -A V2RAY -d 172.16.0.0/12 -j RETURN # 直连局域网，避免 V2Ray 无法启动时无法连网关的 SSH，如果你配置的是其他网段（如 10.x.x.x 等），则修改成自己的
...
iptables -t mangle -A V2RAY_MASK -d 10.0.0.0/8 -j RETURN # 直连局域网
iptables -t mangle -A V2RAY_MASK -d 172.16.0.0/12 -j RETURN # 直连局域网
iptables -t mangle -A V2RAY_MASK -d 192.168.0.0/16 -j RETURN # 直连局域网
